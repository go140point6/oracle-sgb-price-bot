...
async function getInitialPrice() {
  ...
  const wsb = "wsb"
  const ORC_ADDRESS = "0xd7565b16b65376e2ddb6c71e7971c7185a7ff3ff"
  const orc = "orc"
  ...
  let orcSGBPrice = (Number(arryPrice[1]._hex) / 10 ** 18).toFixed(5)
  let wsbUSDPrice = (sgbPrice / wsbSGBPrice).toFixed(5)
  let orcUSDPrice = (sgbPrice / orcSGBPrice).toFixed(5)
  console.log(wsb + " USD price is " + wsbUSDPrice)
  console.log(orc + " USD price is " + orcUSDPrice)

  clearRoles()
  lastPrice = orcUSDPrice || 0
  let symbol = `${process.env.COIN_ID.toUpperCase()}`
  client.user.setPresence({
    activities: [{
    name: `SGB=${orcSGBPrice} ${symbol}`,
    type: `PLAYING`
    }]
  })

  arrow = mid
  client.guilds.cache.find(guild => guild.id === process.env.SERVER_ID).me.setNickname(`${symbol} ${arrow} ${process.env.CURRENCY_SYMBOL}${orcUSDPrice}`)
